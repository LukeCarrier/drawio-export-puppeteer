...
const FORMAT_VSDX = "vsdx";
const FORMATS = [FORMAT_JPEG, FORMAT_PDF, FORMAT_PNG, FORMAT_SVG, FORMAT_VSDX];
...
async function exportVsdx(exporter, input, pageIndex) {
  const { browser, browserTimeout, page } = exporter;

  const vsdx = await page.evaluate(
    async (initArgs) => {
      monkeypatchEditor();
      const editorUi = initEditor(...initArgs);
      try {
        return await exportVsdx(editorUi);
      } catch(err) {
        console.error("Failed to export VSDX", err);
      }
    },
    [input, pageIndex, FORMAT_VSDX]
  );
...
async function exportDiagram(input, pageIndex, format) {
  ...
  switch (format) {
    ...
    case FORMAT_VSDX:
      return exportVsdx(exporter, input, pageIndex);
  }
}
